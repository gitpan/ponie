...
MAGIC** Perl_macro_SvMAGIC (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVMG* data = (XPVMG*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xmg_magic);
  }
  return &((XPVMG*)SvANY(sv))->xmg_magic;
}

HV** Perl_macro_SvSTASH (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVMG* data = (XPVMG*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xmg_stash);
  }
  return &((XPVMG*)SvANY(sv))->xmg_stash;
}


SV** Perl_macro_SvRV (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XRV* data = (XRV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xrv_rv);
  }
...
UV* Perl_macro_SvUVX (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVUV* data = (XPVUV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xuv_uv);
  }
...
NV* Perl_macro_SvNVX (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVNV* data = (XPVNV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xnv_nv);
  }
...
IV* Perl_macro_SvIVX (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIV* data = (XPVIV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xiv_iv);
  }
...
STRLEN* Perl_macro_SvLEN (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPV* data = (XPV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xpv_len);
  }
  return &((XPV*)SvANY(sv))->xpv_len;
  
}

STRLEN* Perl_macro_SvCUR (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPV* data = (XPV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xpv_cur);
  }
...
U8* Perl_macro_BmRARE (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVBM* data = (XPVBM*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xbm_rare);
  }
  return &((XPVBM*)SvANY(sv))->xbm_rare;
}

I32* Perl_macro_BmUSEFUL (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVBM* data = (XPVBM*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xbm_useful);
  }
  return &((XPVBM*)SvANY(sv))->xbm_useful;
}

U16* Perl_macro_BmPREVIOUS (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVBM* data = (XPVBM*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xbm_previous);
  }
  return &((XPVBM*)SvANY(sv))->xbm_previous;
}

IV* Perl_macro_FmLINES (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVFM* data = (XPVFM*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xfm_lines);
  }
  return &((XPVFM*)SvANY(sv))->xfm_lines;
}
...
STRLEN* Perl_macro_LvTARGOFF (pTHX_ SV* sv) {
  XPVLV* data;
  if(!SvPMC(sv))
    abort();  
  data = (XPVLV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));  
  return &(data->xlv_targoff);
}

STRLEN* Perl_macro_LvTARGLEN (pTHX_ SV* sv) {
  XPVLV* data;
  if(!SvPMC(sv))
    abort();
  data = (XPVLV*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));  
  return &(data->xlv_targlen);  
}

PerlIO** Perl_macro_IoIFP (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_ifp);
  }
  return &((XPVIO*)SvANY(sv))->xio_ifp;
}

PerlIO** Perl_macro_IoOFP (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_ofp);
  }
  return &((XPVIO*)SvANY(sv))->xio_ofp;
}

DIR** Perl_macro_IoDIRP (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_dirp);
  }
  return &((XPVIO*)SvANY(sv))->xio_dirp;
}

void** Perl_macro_IoANY (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_any);
  }
  return &((XPVIO*)SvANY(sv))->xio_any;
}

IV* Perl_macro_IoLINES (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_lines);
  }
  return &((XPVIO*)SvANY(sv))->xio_lines;
}

IV* Perl_macro_IoPAGE (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_page);
  }
  return &((XPVIO*)SvANY(sv))->xio_page;
}

IV* Perl_macro_IoPAGE_LEN (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_page_len);
  }
  return &((XPVIO*)SvANY(sv))->xio_page_len;
}

IV* Perl_macro_IoLINES_LEFT (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_lines_left);
  }
  return &((XPVIO*)SvANY(sv))->xio_lines_left;
}

char** Perl_macro_IoTOP_NAME (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_top_name);
  }
  return &((XPVIO*)SvANY(sv))->xio_top_name;
}

GV** Perl_macro_IoTOP_GV (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_top_gv);
  }
  return &((XPVIO*)SvANY(sv))->xio_top_gv;
}

char** Perl_macro_IoFMT_NAME (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_fmt_name);
  }
  return &((XPVIO*)SvANY(sv))->xio_fmt_name;
}

GV** Perl_macro_IoFMT_GV (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_fmt_gv);
  }
  return &((XPVIO*)SvANY(sv))->xio_fmt_gv;
}

char** Perl_macro_IoBOTTOM_NAME (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_bottom_name);
  }
  return &((XPVIO*)SvANY(sv))->xio_bottom_name;
}

GV** Perl_macro_IoBOTTOM_GV (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_bottom_gv);
  }
  return &((XPVIO*)SvANY(sv))->xio_bottom_gv;
}

short* Perl_macro_IoSUBPROCESS (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_subprocess);
  }
  return &((XPVIO*)SvANY(sv))->xio_subprocess;
}

char* Perl_macro_IoTYPE (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_type);
  }
  return &((XPVIO*)SvANY(sv))->xio_type;
}

char* Perl_macro_IoFLAGS (pTHX_ SV *sv) {
  if(SvPMC(sv)) {
    XPVIO* data = (XPVIO*) Parrot_PMC_get_pointer(PL_Parrot, SvANY(sv));
    return &(data->xio_flags);
  }
